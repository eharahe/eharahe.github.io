...
尼尔的美术风格和表现形式的跨度都较大。从恢弘大气的战斗场面，到宁静祥和的遗迹村落，再到欢乐诡异的主题乐园；从2D固定视角的空战，到森林城堡的类横板跳跃，再到正常的3D镜头动作叙事。要Cover这么复杂的美术风格、交互类型和叙事要求，还要做到音乐情绪的连续且自然的表达不是一件容易的事情。

从音乐风格上看，采用了以日式风格的管弦为主，世界民族乐器为辅，大量vocal的使用来做情绪带入的方式；整体给人的感受是即大气又细腻，出色的完成游戏叙事的音乐需求。为了表达整个gameplay背后的设定，音乐色调整体上偏“冷”。作者通过大量冷色调的钢琴、mallet、pad音色的选择，以及突出的延时和混响来营造空间感，希望能表现一个宁静、美丽而又孤独的“新世界”。
...
* 采用了state或RTPC的方式，让Camera距离影响音乐的播放音量的Ducking，比如工厂横板战斗时摄像机的镜头会影响音乐的响度；
* 采用了state和lpf的方式，当菜单呼出时将音乐压低。

#### 垂直分层   

* 器乐多分为3层。偏氛围的分层以合成Pad和钢琴为主；基础分层主要是弦乐、管乐、吉他等和声乐器，或者是名族乐器与小打，创造主要的旋律和律动；打击乐分层，以大鼓、Timpani和其它低频较饱满的节奏乐器为主。  
* 在器乐分层之上还有独立的Vocal分层，当需要带入情绪时，则会和一个或多个器乐分层同时触发。
* 器乐分层虽然有多层，但出于提升音质和复音数的考量，大多数是预渲染的（而不是在Wwise中混音），而Vocal层则基本是独立的，比较便于调整其混音的音量比例。
