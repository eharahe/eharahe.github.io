...
* 器乐分3个层次，Pad/Drone层，创造氛围，以合成音色和Bass为主；基础层，或者是吉他和其他小打，或者是管弦的织体，创造主要的旋律和律动；Drum层，G.C, Timpani和其他强烈的节奏乐器。  
* 除此之外还有Vocal层，相对独立于上面三个分层，多用于和器乐分层做叠加。  
* 这些分层的混音除了Vocal，大多数是预渲染的（而不是在Wwise中混音），应该主要是出于提升音质的考量；但Vocal层往往是独立的。
* 可以看出垂直分层的策略是以满足叙事性需求为主，以丰富音乐的多样性和随机性为辅，这符合ARPG的实际需求。

#### 关于过渡

* 同一段Music不同分层间的过渡：通过Wwise的时间补偿机制。
...
在同一段BGM中的停留时间平均不大于3次。但是像「遺サレタ場所/斜光 」等主要场景的BGM重复率仍然较高，设计时采用了大量分层增加其丰富性（8轨）。
在长达20分钟的最终Boss战中仍然会有听觉疲劳的感觉。
...
| 終ワリノ音 | Full Arrangements | Boss战第三阶段，机械毛毛虫 | 2:18 | 9 | 2:09 | 2:00 | <1 |
