...
巫师3的音乐资源大致分为基于场景的探索、战斗、做X音乐；及基于任务的叙事、对白音乐两大类。其中基于场景的音乐按不同场景进行资源组织；通过垂直分层的方式实现情绪强弱变化；通过wwise transition机制实现音乐之间的过渡。大部分的探索和战斗音乐都是D调，方便过渡；过渡除了按小节对齐外还有采用Custom Cue的方式。
